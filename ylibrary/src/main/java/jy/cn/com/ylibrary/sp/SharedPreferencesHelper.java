...
public class SharedPreferencesHelper {
    ...
    private class ReloadTask implements Runnable {
        ...
        public void run() {
            ...
            YLogUtil.INSTANCE.iTag(TAG, "获取sp数据--缓存到内存--开始");
            keyValueMap.clear();
            Map<String, ?> map = sharedPreferences.getAll();
            if (map != null) {
                keyValueMap.putAll(map);
            }
            YLogUtil.INSTANCE.iFormatTag(TAG, "获取sp数据--缓存到内存--结束---time：%s ms---size：%d",
...
//            YLogUtil.INSTANCE.iTag(TAG, "正在同步");
    ...
    private class SyncTask implements Runnable {
        ...
        public void run() {
            ...
            YLogUtil.INSTANCE.iTag(TAG, "同步开始--size", storeMap.size());

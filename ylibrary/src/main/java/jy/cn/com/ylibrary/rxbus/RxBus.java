...
public class RxBus {
    ...
    public void register(Object subscriber) {
        YLogUtil.INSTANCE.iTag(TAG, "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@注册@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@--订阅者对象", subscriber);
        //根据对象获取类
        ...
        YLogUtil.INSTANCE.iTag(TAG, "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@注册完成@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@--订阅者对象", subscriber);
    }
    ...
    public void unregister(Object subscriber) {
        YLogUtil.INSTANCE.iTag(TAG, "###################################取消注册###################################--订阅者对象", subscriber);
        //通过订阅者对象，获取对应的事件类型集合
        ...
        if (subscribedTypes != null) {
            for (Class<?> eventType : subscribedTypes) {
                ...
                YLogUtil.INSTANCE.iTag(TAG, "执行取消注册--订阅者对象", subscriber, "事件类型", eventType);
            }
            eventTypesBySubscriber.remove(subscriber);
        }
        YLogUtil.INSTANCE.iTag(TAG, "###################################取消注册完成###############################--订阅者对象", subscriber);
    }
    ...
    public synchronized boolean isRegistered(Object subscriber) {
        ...
        YLogUtil.INSTANCE.iTag(TAG, "************************************判断是否注册******************************--订阅者对象", subscriber, "是？", isRegistered);
        return isRegistered;
    }
    ...
    private void executeRegistered(Object subscriber, Method method, Class eventType) {
        ...
        if (isPrimitive) {
            YLogUtil.INSTANCE.iTag(TAG, "*********基础类型自动装箱---开始*********装箱前类型", eventType);
            eventType = PrimitiveWrapper.wrapper(eventType);
            YLogUtil.INSTANCE.iTag(TAG, "*********基础类型自动装箱---完成*********装箱后类型", eventType);
        }
        ...
        YLogUtil.INSTANCE.iTag(TAG, "执行注册---方法:", method, "---事件类型:", eventType);
    }
    ...
    private void callEvent(SubscriberMethod subscriberMethod, Object object) {
        YLogUtil.INSTANCE.iTag(TAG, "执行回调----订阅者对象", subscriberMethod.subscriber, "方法", subscriberMethod.method, "事件类型对象", object);
        //因为最终发送的为事件类型对象
        ...
        if (subscriberMethodList != null && subscriberMethodList.size() > 0) {
            for (SubscriberMethod tmpSubscriberMethod : subscriberMethodList) {
                if (tmpSubscriberMethod.code == subscriberMethod.code && subscriberMethod.subscriber.equals(tmpSubscriberMethod.subscriber)
                    ...
                    YLogUtil.INSTANCE.iTag(TAG, "回调成功----订阅者对象", subscriberMethod.subscriber, "方法", subscriberMethod.method, "事件类型对象", object);
                }
    ...
    public void send(int code, Object o) {
        ...
        bus.onNext(message);
        YLogUtil.INSTANCE.iTag(TAG, "发送RxBus", o);
    }
    ...
    public void send(int code) {
        ...
        bus.onNext(message);
        YLogUtil.INSTANCE.iTag(TAG, "发送RxBus", message.object);
    }

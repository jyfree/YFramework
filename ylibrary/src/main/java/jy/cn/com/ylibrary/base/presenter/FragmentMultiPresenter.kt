package jy.cn.com.ylibrary.base.presenter

import jy.cn.com.ylibrary.base.contract.FragmentMultiContract
 ...
 */
class FragmentMultiPresenter<V : FragmentMultiContract.View, W : FragmentMultiContract.Model> : BasePresenter<V, W>(), FragmentMultiContract.Presenter<V, W> {
        ...
        try {
            mView?.let {
                ...
                if (it.currentTabIndex != position) {
                    ...
                    if (!fragments[position].isAdded && null == it.mFragmentManager.findFragmentByTag(position.toString())) {
                        if (it.showingFragment != null) {
                            trx.hide(fragments[it.currentTabIndex]).add(it.fragmentId, fragments[position], position.toString())
                        } else {

...
public class RomUtil {
    ...
    private static boolean check(String rom) {
        ...
        String tmpName = SharedPreferencesConfigUtils.getInstance(BaseApplication.getInstance()).getString(SharedPreferencesConfigUtils.ROM);
        String tmpVersion = SharedPreferencesConfigUtils.getInstance(BaseApplication.getInstance()).getString(SharedPreferencesConfigUtils.ROM_VERSION);
        if (null != tmpName && !tmpName.isEmpty()) {
            sName = tmpName;
            sVersion = tmpVersion;
            YLogUtil.INSTANCE.iFormatTag(TAG, "sp--name：%s--version：%s---rom：%s", sName, sVersion, rom);
            return sName.equals(rom);
        }
        ...
        SharedPreferencesConfigUtils.getInstance(BaseApplication.getInstance()).setString(SharedPreferencesConfigUtils.ROM, sName);
        SharedPreferencesConfigUtils.getInstance(BaseApplication.getInstance()).setString(SharedPreferencesConfigUtils.ROM_VERSION, sVersion);

        YLogUtil.INSTANCE.iFormatTag(TAG, "getProp--name：%s--version：%s---rom：%s", sName, sVersion, rom);
        return sName.equals(rom);
    }
